...
def fit(train_loader,
    ...
    if checkpoint_weights is not None:
        model, optimizer, start_epoch = load_checkpoint(model, optimizer, scheduler, checkpoint_weights)
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        if scheduler is not None:
            if isinstance(scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                scheduler.step(test_losses_epoch)
            else:
                scheduler.step()
            print('Now, the LR is {}'.format(optimizer.param_groups[0]['lr']))
        
        if snapshot_path is not None:
            if test_losses_epoch < old_val_loss:
                print("Val loss lowered, saving weights of this epoch!\n")
